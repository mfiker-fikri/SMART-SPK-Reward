        ...
        $(".mercadoCountdown1").each( function(){
            ...
            .on('finish.countdown', function(){
                Swal.fire({
                    ...
                    didClose: () => {
                        window.location = "{{ url('form-representative/list') }}";
                    },
                });
            });
        });
        ...
        <div class="col-xxl">
            ...
            <div class="card my-4">
                ...
                <div class="card-header d-flex align-items-center justify-content-between">
                    <h5 class="mb-0">Update Form Pendaftaran Penghargaan Teladan</h5>
                </div>
                <!--/ Form Inovation Update Title -->
                ...
                <div class="card-body py-md-4 py-4 mx-4 mx-4">
                    ...
                    <form id="formRepresentativeUpdate" class="mx-3 my-3" method="POST" action="{{ route('pegawai.postTeladanIdUpdate.Update.Pegawai', $rewardTeladan->id) }}" enctype="multipart/form-data">
                        @csrf
                        ...
                        <div class="row my-3 {{ $errors->has('uploadFile') || $errors->has('uploadFileUpdate') ? 'is-invalid' : '' }}">
                            ...
                            <div class="col-md-9 col-xl-9">
                                <div class="input-group {{ $errors->has('uploadFile') || $errors->has('uploadFileUpdate') ? 'is-invalid' : '' }}">
                                    <input type="hidden" name="oldFile" value="{{ old('oldFile',$rewardTeladan->upload_file_short_description) }}" />
                                    <input type="hidden" name="oldFile" id="oldFile" value="{{ asset('storage/employees/documents/requirementsEmployeesRewardTeladans/'. Auth::guard('employees')->user()->username. '/' . $rewardTeladan->upload_file_short_description) }}" />
                                    ...
                                    <input type="file" class="form-control {{ $errors->has('uploadFile') || $errors->has('uploadFileUpdate')  ? 'is-invalid' : '' }}" id="uploadFile"
                                        value="{{ old('uploadFileUpdate', $rewardTeladan->upload_file_short_description) }}"
                                        name="uploadFile" accept=".pdf" onchange="preview_pdf(event)">

                                    <input type="hidden" name="uploadFileUpdate" id="uploadFileUpdate"
                                        value="{{ $rewardTeladan->upload_file_short_description }}"
                                        required />
                                </div>
                                ...
                                <div class="d-flex justify-content-center py-sm-3">
                                    <iframe class="d-block rounded" width="600" height="350" id="output_pdf" src="{{ asset('storage/employees/documents/requirementsEmployeesRewardTeladans/'. Auth::guard('employees')->user()->username. '/' . $rewardTeladan->upload_file_short_description) }}"></iframe>
                                </div>
                            </div>
                        </div>
                        <!--/ Upload Short Description -->
                        ...
                        <div class="row my-3 {{ $errors->has('uploadPhoto') || $errors->has('uploadPhotoUpdate') ? 'is-invalid' : '' }}">
                            ...
                            <div class="col-md-9 col-xl-9">
                                <div class="input-group {{ $errors->has('uploadPhoto') || $errors->has('uploadPhotoUpdate') ? 'is-invalid' : '' }}">
                                    <input type="hidden" name="oldPhoto" value="{{ old('oldPhoto',$rewardTeladan->upload_file_image_support) }}" />
                                    <input type="hidden" name="oldPhoto" id="oldPhoto" value="{{ asset('storage/employees/images/requirementsEmployeesRewardTeladans/'. Auth::guard('employees')->user()->username. '/' . $rewardTeladan->upload_file_image_support) }}" />
                                    ...
                                    <input type="file" class="form-control {{ $errors->has('uploadPhoto') || $errors->has('uploadPhotoUpdate') ? 'is-invalid' : '' }}" id="uploadPhoto"
                                        value="{{ old('uploadPhotoUpdate', $rewardTeladan->upload_file_image_support) }}"
                                    ...
                                    <input type="hidden" name="uploadPhotoUpdate" id="uploadPhotoUpdate"
                                        value="{{ $rewardTeladan->upload_file_image_support }}"
                                        required />
                                </div>
                                ...
                                <div class="d-flex justify-content-center py-sm-3">
                                    <img class="d-block rounded" width="450" height="350" id="output_image" src="{{ asset('storage/employees/images/requirementsEmployeesRewardTeladans/'. Auth::guard('employees')->user()->username. '/' . $rewardTeladan->upload_file_image_support) }}">
                                </div>
                            </div>
                        </div>
                        <!--/ Upload Photo -->
                        ...
                        <div class="row my-3 {{ $errors->has('uploadVideo') || $errors->has('uploadVideoUpdate') ? 'is-invalid' : '' }}">
                            ...
                            <div class="col-md-9 col-xl-9">
                                <div class="input-group {{ $errors->has('uploadVideo') || $errors->has('uploadVideoUpdate') ? 'is-invalid' : '' }}">
                                    <input type="hidden" name="oldVideo" value="{{ old('oldVideo',$rewardTeladan->upload_file_video_support) }}" />
                                    <input type="hidden" name="oldVideo" id="oldVideo" value="{{ asset('storage/employees/videos/requirementsEmployeesRewardTeladans/'. Auth::guard('employees')->user()->username. '/' . $rewardTeladan->upload_file_video_support) }}" />
                                    ...
                                    <input type="hidden" name="uploadVideoUpdate" id="uploadVideoUpdate"
                                        value="{{ $rewardTeladan->upload_file_video_support }}"
                                        required />
                                </div>
                                ...
                                <div class="d-flex justify-content-center py-sm-3">
                                    <video class="d-block rounded" width="650" height="350" controls id="video_here" src="{{ asset('storage/employees/videos/requirementsEmployeesRewardTeladans/'. Auth::guard('employees')->user()->username. '/' . $rewardTeladan->upload_file_video_support) }}"></video>
                                </div>
                            </div>
                        </div>
                        <!--/ Upload Video -->
