...
@section('js_footer')
    ...
    {{-- <script>
        ...
        function reset_select2() {
            $('#select-categories').val('"'+data.id+'"');
            $('#select-categories').text('"'+data.text+'"');
        }
    </script> --}}

    <!-- Reset Select2 Role -->
    <script type="text/javascript">
        $(document).on('click', '#resetKriteria', function(e) {
            var old = document.getElementById("oldCategories").getAttribute("value");
            if (old) {
                $('#select-categories').val(old).trigger('change');
            } else {
                $('#select-categories').val(null).trigger('change');
            }
        });
        </script>
        <!--/ Reset Select2 Role -->
    ...
    {{-- <script>
        function reset_select2() {
            $('#select-categories').find("option[selected='"+data.id+"']").text();
            // trigger("'"+data.id"'");
            // $('#select-categories').find("option[selected").val();
            // find("option[selected='"+data.id+"']").trigger('change');
            // find(data.id).trigger('change');
            // find("option[value='"+data.id+"']").length
            // select2('data');
            // $('#select-categories :selected').text();
            // find(':selected').data('placeholder');
            // find(":selected").trigger('change');
...
<div class="container-xxl flex-grow-1 container-p-y">
    ...
    <div class="row mt-3">
        ...
        <div class="col-xxl">
            ...
            <!-- Card Update Criteria -->
            <div class="card mx-4">

                <!-- Form Update Criteria Title -->
                ...
                <div class="card-body py-xl-5 py-sm-5 px-xl-5">
                    ...
                    <form id="formUpdateCriterias" class="mx-2" method="POST" action="{{ route('sdm.postManageCriteriasId.KepalaSubbagianPenghargaanDisiplindanPensiun.Update.SDM',$criteria->id) }}">
                        @csrf
                        <!-- Categories -->
                        <div class="mb-3 row {{ $errors->has('categories') ? 'is-invalid' : '' }}">
                            ...
                            <div class="col-sm-9">
                                <div class="input-group input-group-merge {{ $errors->has('categories') ? 'is-invalid' : '' }}">
                                    ...
                                    <input type="hidden" value="{{ old('categories', $criteria->categorie_id) }}" id="oldCategories" />
                                    <select class="form-select {{ $errors->has('categories') ? 'is-invalid' : '' }}" id="select-categories"
                                        ...
                                        @foreach ($category as $c)
                                            <option value="{{ old('categories',$c->id) }}" {{ $criteria->categorie_id === $c->id ? "selected":"" }}>{{ $c->category }}</option>
                                        @endforeach
                                    </select>
                        ...
                        <div class="d-flex justify-content-end">
                            <div class="justify-content-between">
                                ...
                                <button type="reset" class="btn btn-warning btn-lg" style="color: black" id="resetKriteria">
